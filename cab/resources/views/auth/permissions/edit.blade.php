...
<div>
    ...
    <form class="row" action="{{route('admin.user-management.permissions.update', $permission->id)}}" method="POST" enctype="multipart/form-data">
        <div class="col-md-9">
            <div class="card">
                <div class="card-body">
                    @csrf
                    ...
                    <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
                        ...
                        <input type="text" id="name" name="name" class="form-control" value="{{ $permission->name }}" placeholder="Permission name" required />
                        @if($errors->has('name'))
                    ...
                    <div class="form-group {{ $errors->has('event_url') ? 'has-error' : '' }}">
                        ...
                        <input type="text" id="slug" name="slug" class="form-control" value="{{ $permission->slug }}" placeholder="Permission slug" required />
                        @if($errors->has('slug'))
                        <em class="invalid-feedback">
                            {{ $errors->first('slug') }}
                        </em>
